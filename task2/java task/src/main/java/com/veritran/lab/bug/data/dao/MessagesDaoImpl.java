...
//// public Long createMesssage(Request request) typo erro removed one s
    @Override
    public Long createMessage(Request request) {

        final String sql = "INSERT INTO message ( firstName, lastName, age ) VALUES (?, ?,?)";
        //the statement expects 3 parameters and it received 2
        final String firstName = request.getFirstName();
        final String lastName = request.getFirstName();
    ...
    public void updateMessage(Response response) {
        ...
        getJdbcTemplate().update(sql, response.getDateOfBirth(), response.isFunny(), response.getId());
    }
